"""
This module contains tests for `src/jobs/task2/top_marketing_campaigns_df_api.py`.
"""
import unittest
...
from src.jobs.task2 import top_marketing_campaigns_df_api
from src.shared.logging import Log4jWrapper
from src.shared.spark import start_spark_session


class TopMarketingCampaignsDataFrameAPITests(unittest.TestCase):
    """
    Tests for Spark job described in
    `src/jobs/task2/top_marketing_campaigns_df_api.py`.
    """
    ...
    def test_extract_data(self):
        """
        Test: Function `top_marketing_campaigns_df_api#extract_data()` correctly loads test data.
        """
        expected_columns = ["purchaseId", "purchaseTime", "billingCost", "isConfirmed",
                            "sessionId", "campaignId", "channelId"]

        purchases_attribution_df: DataFrame = top_marketing_campaigns_df_api.extract_data(
    ...
    def test_transform_data(self):
        """
        Test: Function `top_marketing_campaigns_df_api#transform_data()` works correctly.
        """
        # Input data
        ...
        # Actual results
        top_marketing_campaigns_df = top_marketing_campaigns_df_api.transform_data(
    ...
    def test_load_data(self):
        """
        Test: Function `top_marketing_campaigns_df_api#load_data()` saves data without errors.
        """
        # Input data
        dummy_data_df: DataFrame = self.spark_session.read.csv(
            path="tests/_data/dummy/top_marketing_campaigns.csv",
        ...
        top_marketing_campaigns_df_api.load_data(dummy_data_df, self.config)

    # pylint: disable=no-self-use
    def test_main(self):
        """
        Test: Job `top_marketing_campaigns_df_api` is completing without errors on test data.
        """
        top_marketing_campaigns_df_api.main()
